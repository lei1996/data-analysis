    ...
    new Observable<ResultInterface>(
      (subscriber: Subscriber<ResultInterface>) => {
        ...
        const sourcePipe = (
          ...
          return pipe(
            switchMap(([item, rsiResult]) =>
              best.pipe(switchMap((x) => of({ rsiResult, item, best: x }))),
            ),
              ...
              return from(bests).pipe(
                concatMap((best) => {
                  const last$ = zip(from(klines), from(maxminArrs)).pipe(
                    concatMap(([item, rsiResult]) => {
                      return of({ rsiResult, item, best });
                    }),
                    operator(),
                    concatMap(({ item, info }) =>
                      of({
      ...
      (subscriber: Subscriber<OperatorResultInterface>) => {
        ...
        const subscription = observable.subscribe({
          next({ rsiResult, best, item }) {
            ...
            if (!!info) {
              if (!send.isOpen && diff.isStable && info.includes('开')) {
                let stop = new Big(0);
      ...
      (subscriber: Subscriber<OperatorResultInterface>) => {
        ...
        const subscription = observable.subscribe({
          next({ rsiResult, best, item }) {
            ...
            if (!!info) {
              if (diff.isStable && info.includes('开')) {
                let stop = new Big(0);
